...
public class UrlsController: ControllerBase
{
    ...
    public UrlsController(IMediator mediator, IHeimGuardClient heimGuard)
    {
    ...
    public async Task<ActionResult<UrlDto>> AddUrl([FromBody]UrlForCreationDto urlForCreation)
    {
        var command = new AddUrl.AddUrlCommand(urlForCreation);
        var commandResponse = await _mediator.Send(command);

        return CreatedAtRoute("GetUrl",
    ...
    [ProducesResponseType(500)]
    [Produces("application/json")]
    [HttpGet("{id:guid}", Name = "GetUrl")]
    public async Task<ActionResult<UrlDto>> GetUrl(Guid id)
    {
        var query = new GetUrl.UrlQuery(id);
    ...
    /// </remarks>
    [ProducesResponseType(typeof(IEnumerable<UrlDto>), 200)]
    [ProducesResponseType(400)]
    [ProducesResponseType(500)]
    [Produces("application/json")]
    [HttpGet(Name = "GetUrls")]
    public async Task<IActionResult> GetUrls([FromQuery] UrlParametersDto urlParametersDto)
    {
        var query = new GetUrlList.UrlListQuery(urlParametersDto);
        var queryResponse = await _mediator.Send(query);
    ...
    [ProducesResponseType(500)]
    [Produces("application/json")]
    [HttpPut("{id:guid}", Name = "UpdateUrl")]
    public async Task<IActionResult> UpdateUrl(Guid id, UrlForUpdateDto url)
    {
        var command = new UpdateUrl.UpdateUrlCommand(id, url);
    ...
    [ProducesResponseType(500)]
    [Produces("application/json")]
    [HttpDelete("{id:guid}", Name = "DeleteUrl")]
    public async Task<ActionResult> DeleteUrl(Guid id)
    {
        var command = new DeleteUrl.DeleteUrlCommand(id);
        await _mediator.Send(command);
